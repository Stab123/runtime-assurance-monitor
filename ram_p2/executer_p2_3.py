...
tirages, mêmes fonctions. La parallélisation porte uniquement sur les runs,
indépendants par construction : paramètres de plante tirés de graine_plante + i,
bruit de graine_bruit + i, sans état partagé. L'ordre est préservé (imap
ordonné), donc les listes par point de grille sont identiques à une exécution
série.

Validation du pilote : relancé avec n_runs=32 sur la config P2.2, il doit
reproduire resultats_p2_2.json bit à bit.
...
def main(chemin_config=None, chemin_sortie=None, n_proc: int = 2):
    ...
    lignes.append(f"Campagne {cfg.get('version_config', '?')} — "
                  f"{n_runs} runs par point (execution parallele, {n_proc} processus)")
    lignes.append(f"CRN : {crn_ok}/{n_runs} runs avec temoin identique "
                  f"sur les quatre bras ({'OK' if crn_ok == n_runs else 'ECHEC'})")
    lignes.append(f"Duree de calcul : {duree:.0f} s")
    lignes.append(f"{'bras':<4} {'seuil_sig_soc':>13} {'N':>3} "
    ...
    brut = {
        "config": cfg,
        "empreintes_code": empreinte_fichiers(MODULES_EMPREINTE_P2_3),
        "crn_runs_ok": crn_ok,
        "resultats_par_run": {
